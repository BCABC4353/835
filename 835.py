...
# Global Fair Health rates instance
_fair_health_rates = None

# Trips.csv path and lookup dictionary
TRIPS_CSV_PATH = r"C:\Users\Brendan Cameron\Desktop\Trips.csv"
_trips_by_run = {}  # RUN -> normalized ZIP mapping
...
def load_fair_health_rates(rates_file_path=None):
    ...
    """
    global _fair_health_rates
    
    if rates_file_path is None:
        # Default location: user's Desktop
        rates_file_path = os.path.expanduser(r"~\Desktop\RATES.xlsx")
    
    if not os.path.exists(rates_file_path):
        print(f"  Note: Fair Health rates file not found: {rates_file_path}")
        print(f"        Fair Health rate columns will be empty.")
        return False
    
    try:
        _fair_health_rates = rates.FairHealthRates()
        stats = _fair_health_rates.load_from_excel(rates_file_path)
        print(f"  Loaded Fair Health rates: {stats['rate_keys']} rate combinations")
        print(f"    ZIP codes: {stats['unique_zips']}, HCPCS codes: {stats['unique_hcpcs']}")
        return True
    except Exception as e:
        print(f"  Warning: Failed to load Fair Health rates: {e}")
        _fair_health_rates = None
        return False
...
def load_trips_csv():
    ...
    global _trips_by_run
    if not os.path.exists(TRIPS_CSV_PATH):
        print(f"  Note: Trips.csv not found at {TRIPS_CSV_PATH}")
        return False
    ...
    with open(TRIPS_CSV_PATH, 'r') as f:
        ...
        for row in reader:
            ...
            if run and puzip:
                ...
                if normalized:
                    _trips_by_run[run] = normalized
    print(f"  Loaded {len(_trips_by_run)} trip records from Trips.csv")
    return True
...
def get_fair_health_rates_for_service(hcpcs_code, service_date_str):
    ...
    return _fair_health_rates.get_rates_for_all_zips(hcpcs_code, service_date)
...
def parse_cas(elements, segment_id='', claim_status=None):
    """
...
def parse_ref(elements):
    return {
        'qualifier': elements[1] if len(elements) > 1 else '',
        'qualifier_desc': get_reference_qualifier_description(elements[1]) if len(elements) > 1 else '',
        'ref_value': elements[2] if len(elements) > 2 else '',
        'description': elements[3] if len(elements) > 3 else ''
    }


def parse_dtm(elements):
    return {
        'date_qualifier': elements[1] if len(elements) > 1 else '',  
        'date_qualifier_desc': get_date_qualifier_description(elements[1]) if len(elements) > 1 else '',
...
def parse_nm1(elements):
    return {
        'entity_id_code': elements[1] if len(elements) > 1 else '',
        'entity_id_desc': get_entity_identifier_description(elements[1]) if len(elements) > 1 else '',
...
# Import categorize_adjustment from the new module
    ...
    for seg_idx, seg in enumerate(segments):
        ...
        if seg_id == 'ST' and len(elements) > 1:
            ...
            if not current_claim and not current_service:
                ...
                dtm_data = parse_dtm(elements)
                current_claim['_dtm'].append(dtm_data)
            elif current_service and current_claim:
                current_service['_dtm'].append(parse_dtm(elements))
            ...
            if not claim_number:
                # Generate a unique identifier using position in file
                claim_number = f'EMPTY_CLAIM_{len(all_rows)+1}'
            ...
            if entity_code == 'PR':
                ...
                current_claim['claim_payer_id'] = nm1_data.get('id_number', '')    # NM109
                current_claim['claim_payer_id_qualifier'] = nm1_data.get('id_qualifier', '')  # NM108
                ...
                try:
                    ...
                    for i in range(seg_idx + 1, min(seg_idx + 4, len(segments))):
                        ...
                        if next_seg_id == 'N3':
                            ...
                            current_claim['_ambulance_pickup_city'] = n4_data.get('city_name', '')
                            current_claim['_ambulance_pickup_state'] = n4_data.get('state_code', '')
                ...
                try:
                    ...
                    for i in range(seg_idx + 1, min(seg_idx + 4, len(segments))):
                        ...
                        if next_seg_id == 'N3':
                            ...
                            n4_data = parse_n4(next_seg)
                            current_claim['_ambulance_dropoff_city'] = n4_data.get('city_name', '')
                            current_claim['_ambulance_dropoff_state'] = n4_data.get('state_code', '')
            ...
            current_claim['_ref'].append(parse_ref(elements))
            ...
            cas_data = safe_parse_segment(seg, element_delimiter,
                                         lambda els: parse_cas(els, 'CLAIM', claim_status),
                                         [])
            if cas_data:
        ...
        # Remove duplicate - already handled earlier
            ...
            claim_status = current_claim.get('claim_status')
            cas_data = safe_parse_segment(seg, element_delimiter,
                                         lambda els: parse_cas(els, 'SERVICE', claim_status),
                                         [])
            if cas_data:
                current_service['_cas'].extend(cas_data)

        elif seg_id == 'REF' and current_service and current_claim:
            current_service['_ref'].append(parse_ref(elements))
            ...
            plb_details = format_plb_details(plb_adjustments)
...
def format_plb_details(plb_list):
    """
    ...
    for plb in plb_list:
        ...
        for adj in plb.get('adjustments', []):
            ...
            if reason_code and amt:
                description = dictionary.get_plb_adjustment_code_description(reason_code)
...
def extract_ref_value(ref_list, qualifier):
    ...
    for ref in ref_list:
        if ref.get('reference_id_qualifier') == qualifier:
...
def create_output_row(claim, service, header_payer_name, payer_address, payer_address2, payer_city, payer_state, payer_zip,
                     ...
                     payee_rdm_data=None, payee_n2_data=None):
    
    # Import categorize_adjustment at the start of the function
    from categorization import categorize_adjustment
    ...
    # Extract QTY values
    # Institutional-only day counts - not used for professional claims
    # covered_actual_days = extract_qty_value(claim.get('_qty', []), 'CA')
    # lifetime_reserve_days = extract_qty_value(claim.get('_qty', []), 'LA')
    # coinsurance_days = extract_qty_value(claim.get('_qty', []), 'CD')
    covered_actual_days = ''
    ...
    if '_nm1' in claim:
        for nm1 in claim['_nm1']:
            if nm1.get('entity_id_code') == '74':
                ...
                corrected_insured_id_qualifier = nm1.get('id_qualifier', '')
    ...
    if '_lq' in claim:
        for lq in claim['_lq']:
            ...
            if code:
                ...
                claim_remark_desc.append(get_remark_code_description(code))
    ...
    if service and '_lq' in service:
        for lq in service['_lq']:
            ...
            if code:
                ...
                service_remark_desc.append(get_remark_code_description(code))
    ...
    row = {
        ...
        'CLM_CAS1_ReasonDesc_L2100_CAS': get_carc_description(claim.get('_cas', [{}])[0].get('reason_code', '')) if claim.get('_cas') and len(claim.get('_cas', [])) > 0 else '',
        ...
        'CLM_CAS2_ReasonDesc_L2100_CAS': get_carc_description(claim.get('_cas', [{}])[1].get('reason_code', '')) if claim.get('_cas') and len(claim.get('_cas', [])) > 1 else '',
        ...
        'CLM_CAS3_ReasonDesc_L2100_CAS': get_carc_description(claim.get('_cas', [{}])[2].get('reason_code', '')) if claim.get('_cas') and len(claim.get('_cas', [])) > 2 else '',
        ...
        'CLM_CAS4_ReasonDesc_L2100_CAS': get_carc_description(claim.get('_cas', [{}])[3].get('reason_code', '')) if claim.get('_cas') and len(claim.get('_cas', [])) > 3 else '',
        ...
        'CLM_CAS5_ReasonDesc_L2100_CAS': get_carc_description(claim.get('_cas', [{}])[4].get('reason_code', '')) if claim.get('_cas') and len(claim.get('_cas', [])) > 4 else '',
        ...
        'CLM_HealthcareRemarkDescriptions_L2100_LQ': '; '.join([get_remark_code_description(c) for c in extract_lq_codes_by_qualifier(claim.get('_lq', []), 'HE')]),
        ...
        'ServiceProvider_EntityCodeDesc_L2100_NM1': get_entity_identifier_description(claim.get('_service_provider', {}).get('entity_code', '')),
        ...
        'SVC_CAS1_ReasonDesc_L2110_CAS': get_carc_description(service['_cas'][0]['reason_code']) if service and '_cas' in service and len(service['_cas']) > 0 else '',
        ...
        'SVC_CAS2_ReasonDesc_L2110_CAS': get_carc_description(service['_cas'][1]['reason_code']) if service and '_cas' in service and len(service['_cas']) > 1 else '',
        ...
        'SVC_CAS3_ReasonDesc_L2110_CAS': get_carc_description(service['_cas'][2]['reason_code']) if service and '_cas' in service and len(service['_cas']) > 2 else '',
        ...
        'SVC_CAS4_ReasonDesc_L2110_CAS': get_carc_description(service['_cas'][3]['reason_code']) if service and '_cas' in service and len(service['_cas']) > 3 else '',
        ...
        'SVC_CAS5_ReasonDesc_L2110_CAS': get_carc_description(service['_cas'][4]['reason_code']) if service and '_cas' in service and len(service['_cas']) > 4 else '',
        ...
        'SVC_HealthcareRemarkDescriptions_L2110_LQ': '; '.join([get_remark_code_description(c) for c in extract_lq_codes_by_qualifier(service.get('_lq', []), 'HE')]) if service else '',
        ...
        'PLB_Details_PLB': format_plb_details(plb_adjustments),
        ...
        'MOA_ClaimPaymentRemarkDesc1_L2100_MOA': get_remark_code_description(claim.get('_moa', [{}])[0].get('claim_payment_remark_code_1', '')) if claim.get('_moa') and claim.get('_moa', [{}])[0].get('claim_payment_remark_code_1') else '',
        'MOA_ClaimPaymentRemarkDesc2_L2100_MOA': get_remark_code_description(claim.get('_moa', [{}])[0].get('claim_payment_remark_code_2', '')) if claim.get('_moa') and claim.get('_moa', [{}])[0].get('claim_payment_remark_code_2') else '',
        'MOA_ClaimPaymentRemarkDesc3_L2100_MOA': get_remark_code_description(claim.get('_moa', [{}])[0].get('claim_payment_remark_code_3', '')) if claim.get('_moa') and claim.get('_moa', [{}])[0].get('claim_payment_remark_code_3') else '',
        'MOA_ClaimPaymentRemarkDesc4_L2100_MOA': get_remark_code_description(claim.get('_moa', [{}])[0].get('claim_payment_remark_code_4', '')) if claim.get('_moa') and claim.get('_moa', [{}])[0].get('claim_payment_remark_code_4') else '',
        'MOA_ClaimPaymentRemarkDesc5_L2100_MOA': get_remark_code_description(claim.get('_moa', [{}])[0].get('claim_payment_remark_code_5', '')) if claim.get('_moa') and claim.get('_moa', [{}])[0].get('claim_payment_remark_code_5') else '',
        ...
        'MIA_ClaimPaymentRemarkDesc_L2100_MIA': get_remark_code_description(claim.get('_mia', [{}])[0].get('claim_payment_remark_code', '')) if claim.get('_mia') and claim.get('_mia', [{}])[0].get('claim_payment_remark_code') else '',
        ...
        'MIA_ClaimPaymentRemarkDesc2_L2100_MIA': get_remark_code_description(claim.get('_mia', [{}])[0].get('claim_payment_remark_code_2', '')) if claim.get('_mia') and claim.get('_mia', [{}])[0].get('claim_payment_remark_code_2') else '',
        'MIA_ClaimPaymentRemarkDesc3_L2100_MIA': get_remark_code_description(claim.get('_mia', [{}])[0].get('claim_payment_remark_code_3', '')) if claim.get('_mia') and claim.get('_mia', [{}])[0].get('claim_payment_remark_code_3') else '',
        'MIA_ClaimPaymentRemarkDesc4_L2100_MIA': get_remark_code_description(claim.get('_mia', [{}])[0].get('claim_payment_remark_code_4', '')) if claim.get('_mia') and claim.get('_mia', [{}])[0].get('claim_payment_remark_code_4') else '',
        'MIA_ClaimPaymentRemarkDesc5_L2100_MIA': get_remark_code_description(claim.get('_mia', [{}])[0].get('claim_payment_remark_code_5', '')) if claim.get('_mia') and claim.get('_mia', [{}])[0].get('claim_payment_remark_code_5') else '',
    ...
    fh_rates = get_fair_health_rates_for_service(hcpcs_code, service_start)
    ...
    # Calculate EDI mileage unit price (charge per mile)
    edi_mileage_unit_price = ''
    if is_mileage_code and service:
        try:
            charge = float(service.get('line_charged', 0) or 0)
            units = float(service.get('units', 0) or 0)
            if units > 0:
                edi_mileage_unit_price = round(charge / units, 2)
        except (ValueError, TypeError):
            pass
    ...
    pickup_zip = _trips_by_run.get(run_number)
    ...
    if pickup_zip and _fair_health_rates:
        # Convert pickup_zip to int for lookup (rates.py uses int keys)
        try:
            zip_int = int(pickup_zip)
        except ValueError:
            zip_int = None
        
        rate_tuple = fh_rates.get(zip_int) if zip_int else None
        if rate_tuple:
            ...
            # Calculate Fair Health unit prices and totals for mileage codes
            if is_mileage_code:
                # Unit prices (rate / 15 miles base)
                oon_unit_price = ''
                in_unit_price = ''
                try:
                    oon_unit_price = round(float(oon_rate) / FH_MILEAGE_BASE_UNITS, 2) if oon_rate else ''
                except (ValueError, TypeError):
                    pass
                try:
                    in_unit_price = round(float(in_rate) / FH_MILEAGE_BASE_UNITS, 2) if in_rate else ''
                except (ValueError, TypeError):
                    pass
                ...
                if oon_unit_price and service_units > 0:
                    try:
                        oon_miles = round(float(oon_unit_price) * service_units, 2)
                    except (ValueError, TypeError):
                        pass
                if in_unit_price and service_units > 0:
                    try:
                        in_miles = round(float(in_unit_price) * service_units, 2)
                    except (ValueError, TypeError):
                        pass
...
def process_folder(folder_path, enable_redaction=False, status_callback=None):
    ...
    if enable_redaction:
        ...
        if testing_folder.exists():
            try:
                ...
                if hasattr(e, 'winerror') and e.winerror == 32:
                    print(f"\nWARNING: Cannot delete existing testing folder (in use by another process)")
                    print(f"Creating a new folder with timestamp instead...")
        ...
        print(f"Created testing folder: {testing_folder}")
    file_pattern = str(folder / '*.835')
    txt_pattern = str(folder / '*.txt')
    edi_pattern = str(folder / '*.edi')
    x12_pattern = str(folder / '*.x12')
    processed_pattern = str(folder / '*.PROCESSED')
    candidate_files = []
    candidate_files.extend(glob.glob(file_pattern))
    candidate_files.extend(glob.glob(txt_pattern))
    candidate_files.extend(glob.glob(edi_pattern))
    candidate_files.extend(glob.glob(x12_pattern))
    candidate_files.extend(glob.glob(processed_pattern))
    if not candidate_files:
        print(f"No 835 files found in {folder_path}")
        return None
    ...
    if skipped_files:
        print(f"\nSkipped {len(skipped_files)} non-EDI file(s):")
        for path, reason in skipped_files:
            print(f"  - {os.path.basename(path)}: {reason}")
    
    if not files:
        print(f"No valid EDI files found in {folder_path}")
        return None
    print(f"\nFound {len(files)} valid EDI file(s) to process")
    
    # Load Fair Health rates
    print("\nLoading Fair Health rates...")
    load_fair_health_rates()
    
    # Load Trips.csv for ZIP lookup
    print("\nLoading Trips.csv for ZIP lookup...")
    load_trips_csv()
    
    all_rows = []
    all_parsed_data = []  # Cache parsed data to avoid re-parsing for validation
            ...
            if parsed_data:
                ...
                if enable_redaction and testing_folder:
                    try:
                        ...
                        print(f"  - Saved redacted EDI: {redacted_file_path.name}")
                        # Use redacted file path for CSV
                        csv_file_path = str(redacted_file_path)
                    except Exception as redact_error:
                        print(f"  - WARNING: Redaction failed: {str(redact_error)}")
                ...
                try:
                    ...
                    print(f"  - Extracted {len(rows)} service lines")
                except Exception as convert_error:
                    print(f"  - ERROR in convert_segments_to_rows: {str(convert_error)}")
                    print(f"  - Error type: {type(convert_error).__name__}")
    ...
    if not all_rows:
        ...
        return None
    if enable_redaction and testing_folder:
        output_file = testing_folder / '835_consolidated_output.csv'
    else:
        output_file = folder / '835_consolidated_output.csv'
    import csv
    ...
    print(f"\nPreprocessing {len(all_rows):,} rows (normalize, detect columns)...")
    
    # Single pass: normalize all rows AND detect populated columns simultaneously
    normalized_rows = []
    ...
    for idx, row in enumerate(all_rows, 1):
        ...
        # Normalize all values: uppercase, strip whitespace, format dates/currency
        row = normalize_csv_row(row)
        if enable_redaction:
            row = redact_csv_row(row)
    ...
    print(f"  Normalization complete. Found {len(populated_columns_set)} populated columns.")
    ...
    if status_callback:
        status_callback(f"Writing standard CSV...")
    print(f"\nSaving consolidated output to: {output_file}")
    print(f"  Writing {len(normalized_rows):,} service lines...")
    ...
    print(f"\nConsolidated output saved: {output_file}")
    print(f"  Total service lines: {len(normalized_rows)}")
    ...
    if normalized_rows:
        ...
        if removed_count > 0:
            if enable_redaction and testing_folder:
                compact_file = testing_folder / '835_consolidated_output_compact.csv'
            else:
                compact_file = folder / '835_consolidated_output_compact.csv'
            
            print(f"\nGenerating compact CSV (removed {removed_count} empty columns)...")
            if status_callback:
            ...
            print(f"\nCompact output saved: {compact_file}")
            print(f"  Total service lines: {len(normalized_rows)}")
            print(f"  Columns: {len(populated_columns)} of {len(all_fieldnames)} ({removed_count} empty columns removed)")
        else:
            print(f"\n  All {len(all_fieldnames)} columns have data - no compact version needed")
    
    # Run zero-fail validation
    print("\nRunning Zero-Fail Validation...")
    if status_callback:
        status_callback("Running Zero-Fail Validation...")
    try:
        # Collect EDI segments with file context (like the parser does)
        # Each entry is (filename, segments, delimiter) so validation can process per-file
        edi_segments_by_file = []
        for idx, parsed_data in enumerate(all_parsed_data):
            file_path = files[idx] if idx < len(files) else f"file_{idx}"
            edi_segments_by_file.append({
                'file': str(file_path),
        ...
        # Run validation
        if edi_segments_by_file and all_rows:
            # Determine validation output path
            if enable_redaction and testing_folder:
                validation_output = testing_folder / '835_validation_report.txt'
                validation_html = testing_folder / '835_validation_report.html'
            else:
                validation_output = folder / '835_validation_report.txt'
                validation_html = folder / '835_validation_report.html'
            
            # Run validation and generate reports
            validation_result = validate_835_output(
                edi_segments_by_file, 
                all_rows,
                element_delimiter=all_parsed_data[-1]['element_delimiter'] if all_parsed_data else '*',
            ...
            print(f"  Total Fields Validated: {summary.get('fields_validated', 0):,}")
            print(f"  Calculations Checked: {summary.get('calculations_checked', 0):,}")
            ...
            if payer_quality_issues:
                print("\n" + "=" * 80)
                print("PAYER DATA QUALITY ISSUES")
                print("=" * 80)
                ...
                for issue_type in sorted(issue_types.keys()):
                    ...
                    for payer_key, count in payer_list:
                        ...
                        print(f"  - {payer_name} ({payer_state}): {count:,}")
            else:
                ...
                if payers_missing_units:
                    print("\nPayer Data Quality Issues (Missing Mileage Units):")
                    ...
                    for payer, count in payer_list:
                        ...
                        payer_name = payer_parts[0]
                        payer_state = payer_parts[1] if len(payer_parts) > 1 else 'N/A'
                        print(f"  - {payer_name} ({payer_state}): {count} service line(s)")
            ...
            if errors > 0:
                ...
                print("\n" + "=" * 80)
                print("ERROR ANALYSIS - GROUPED BY PROVIDER STATE → PROVIDER → PAYER → ERROR TYPE")
                print("(Showing sample of 10 errors per type - full list in report files)")
                print("=" * 80)
                ...
                for state in sorted(state_totals.keys(), key=lambda s: state_totals[s], reverse=True):
                    ...
                    print(f"║  Total Errors: {state_totals[state]:,}")
                    ...
                    for provider in sorted(provider_totals.keys(), key=lambda p: provider_totals[p], reverse=True):
                        ...
                        print(f"║  │ Total Errors: {provider_totals[provider]:,}")
                        ...
                        for payer in sorted(payer_totals.keys(), key=lambda p: payer_totals[p], reverse=True):
                            ...
                            print(f"║  │")
                            print(f"║  ├──┬ PAYER: {payer}")
                            print(f"║  │  │ Errors: {payer_totals[payer]:,}")
                            ...
                            for error_type in sorted(types.keys(), key=lambda t: len(types[t]), reverse=True):
                                ...
                                if error_type == 'COMPOSITE_PARSE' and len(type_errors) > 5:
                                    ...
                                    print(f"║  │  │    Summary: {len(claims_affected)} claims with composite errors")
                                    for i, (claim_id, service_count) in enumerate(list(claims_affected.items())[:10], 1):
                                        print(f"║  │  │    {i}. Claim {claim_id}: {service_count} service(s)")
                                    if len(claims_affected) > 10:
                                        print(f"║  │  │    ... and {len(claims_affected) - 10} more claims")
                                else:
                                    ...
                                    if len(type_errors) > 10:
                                        print(f"║  │  │    ... and {len(type_errors) - 10:,} more errors")
            ...
            if warnings > 0:
                ...
                print("(Showing sample of 10 per payer - full list in report files)")
                print("-" * 80)
                ...
                for payer_key in sorted(warnings_by_payer.keys()):
                    ...
                    print(f"\n{payer_key}: {len(payer_warnings):,} warning(s)")
                    ...
                    for msg, instances in msg_types.items():
                        print(f"  {msg}: {len(instances):,} instance(s)")
                        ...
                        if claim_ids:
                            print(f"    Sample claims: {', '.join(claim_ids)}")
                        if len(instances) > 10:
                            print(f"    ... and {len(instances) - 10:,} more")
            
            print("\n" + "=" * 80)
            print("DETAILED REPORTS GENERATED:")
            print(f"  Text Report: {validation_output}")
            print(f"  HTML Report: {validation_html}")
            print("=" * 80)
        ...
        print(f"\nWARNING: Validation failed with error: {str(e)}")
        print(f"  Error type: {type(e).__name__}")
        print("\n  Full traceback:")
        for line in traceback.format_tb(e.__traceback__):
            print(f"    {line.rstrip()}")
        print("\n  CSV output was still generated successfully")
    ...
    if unmapped_elements or unmapped_qualifiers:
        print("\n" + "=" * 100)
        print("EDI ELEMENT COVERAGE ANALYSIS")
        print("These EDI fields have data but are not currently extracted to CSV:")
        print("=" * 100)
        ...
        if unmapped_elements:
            ...
            for key, info in unmapped_elements.items():
                ...
                if info['files_with_data'] >= 5 or info['pct'] >= 1.0:
                    print(f"\n  {key}: {info['files_with_data']:,} files ({info['pct']}%) - {info['total_occurrences']:,} total occurrences")
                    print(f"       Description: {info['description']}")
                    if info['payers']:
                        payer_list = ', '.join(f"{p[0]} ({p[1]:,})" for p in info['payers'][:5])
                        print(f"       Top Payers: {payer_list}")
                        if len(info['payers']) > 5:
                            print(f"                   ... and {len(info['payers']) - 5} more payers")
                    shown += 1
            if shown == 0:
                print("  (No significant unmapped elements - all rare occurrences)")
            elif shown < len(unmapped_elements):
                print(f"\n  ... plus {len(unmapped_elements) - shown} rare elements (< 5 files or < 1%)")
        
        # Report qualifier-based unmapped fields
        if unmapped_qualifiers:
            print("\n--- QUALIFIER-BASED SEGMENTS (REF/DTM/AMT/QTY) ---")
            shown = 0
            for key, info in unmapped_qualifiers.items():
                # Only show qualifiers present in at least 1% of files or at least 5 files
                if info['files_with_data'] >= 5 or info['pct'] >= 1.0:
                    print(f"\n  {key}: {info['files_with_data']:,} files ({info['pct']}%) - {info['total_occurrences']:,} total occurrences")
                    print(f"       Segment: {info['segment']}, Qualifier: {info['qualifier']}")
                    if info['payers']:
                        payer_list = ', '.join(f"{p[0]} ({p[1]:,})" for p in info['payers'][:5])
                        print(f"       Top Payers: {payer_list}")
                        if len(info['payers']) > 5:
                    ...
                    shown += 1
            if shown == 0:
                print("  (No significant unmapped qualifiers - all rare occurrences)")
            elif shown < len(unmapped_qualifiers):
                print(f"\n  ... plus {len(unmapped_qualifiers) - shown} rare qualifiers (< 5 files or < 1%)")
        
        print("\n" + "=" * 100)
    
    if enable_redaction and testing_folder:
        print(f"\nRedacted files (EDI + CSV) saved to: {testing_folder}")
        print(f"   Original files remain unchanged in: {folder}")
