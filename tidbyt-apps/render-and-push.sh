#!/usr/bin/env bash
# Render a .star applet and push it to your Tidbyt device (one-time display).
# Usage: ./render-and-push.sh <applet.star>

set -euo pipefail

if [ $# -lt 1 ]; then
    echo "Usage: $0 <applet.star>"
    exit 1
fi
...
echo "Pushing $WEBP_FILE to device $TIDBYT_DEVICE_ID..."
pixlet push "$TIDBYT_DEVICE_ID" "$WEBP_FILE" -t "$TIDBYT_API_TOKEN"

echo "Done! Check your Tidbyt."
