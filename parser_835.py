...
from validation import EDIElementPresenceTracker, EDIFieldTracker, generate_validation_report, validate_835_output
...
def parse_ref(elements, payer_key=None):
    ...
    return {
        ...
        "qualifier_desc": get_reference_qualifier_description(elements[1], payer_key=payer_key)
        if len(elements) > 1
        else "",
...
def parse_dtm(elements, payer_key=None):
    ...
    return {
        ...
        "date_qualifier_desc": get_date_qualifier_description(elements[1], payer_key=payer_key)
        if len(elements) > 1
        else "",
...
def parse_nm1(elements, payer_key=None):
    ...
    return {
        ...
        "entity_id_desc": get_entity_identifier_description(elements[1], payer_key=payer_key)
        if len(elements) > 1
        else "",
...
def parse_per(elements):
    ...
    for i in range(3, len(elements), 2):
        if i + 1 < len(elements):
            communication_pairs.append({"qualifier": elements[i], "number": elements[i + 1]})
...
def parse_lx(elements):
    return {"header_number": elements[1] if len(elements) > 1 else ""}
...
def is_transportation_code(code):
    ...
    if not code:
        return False
    return code.startswith("A0") or code.startswith("T2") or code in ("S0207", "S0208", "S0209", "S0215")
...
def extract_subscriber_info(claim):
    ...
    if "_nm1" in claim:
        for nm1 in claim["_nm1"]:
            if nm1.get("entity_id_code") == "IL":
                name_parts = [nm1.get("last_name", ""), nm1.get("first_name", ""), nm1.get("middle_name", "")]
    ...
    return {"MemberID": member_id, "SubscriberName": subscriber_name, "PatientName": patient_name, "SSN": ssn}
...
def extract_payer_info(claim):
    ...
    return {"PayerName": payer_name, "PayerID": payer_id}
...
def identify_payer(segments, element_delimiter):
    ...
    payer_info["colloquial_payer_key"] = colloquial.identify_payer(
        trn03=payer_info.get("trn03"), payer_name=payer_info.get("payer_name"), isa06=payer_info.get("isa06")
    )
...
def convert_segments_to_rows(segments, element_delimiter, file_name, component_delimiter=None):
    ...
    for _idx, seg in enumerate(segments):
        ...
        if elements[0] == "PLB":
            plb_entry = safe_parse_segment(seg, element_delimiter, lambda els: parse_plb(els, component_delimiter), {})
            if plb_entry:
    ...
    for seg_idx, seg in enumerate(segments):
        ...
        if seg_id == "ST" and len(elements) > 1:
            ...
            current_claim = {
                ...
                "filing_indicator_desc": get_claim_filing_indicator_description(elements[6])
                if len(elements) > 6
                else "",
            ...
            cas_data = safe_parse_segment(
                seg, element_delimiter, lambda els: parse_cas(els, "CLAIM", claim_status, payer_key=payer_key), []
            )
            if cas_data:
        ...
        # Remove duplicate - already handled earlier
            ...
            if len(elements) > 1:
                current_claim["_service_provider"] = {
                    ...
                    "entity_relationship_desc": get_entity_relationship_description(elements[5])
                    if len(elements) > 5
                    else "",
            ...
            claim_status = current_claim.get("claim_status")
            cas_data = safe_parse_segment(
                seg, element_delimiter, lambda els: parse_cas(els, "SERVICE", claim_status, payer_key=payer_key), []
            )
            if cas_data:
...
def create_output_row(
    ...
    if "_nm1" in claim:
        for nm1 in claim["_nm1"]:
            if nm1.get("entity_id_code") == "82":
                name_parts = [nm1.get("last_name", ""), nm1.get("first_name", "")]
    ...
    row = {
        ...
        "CHK_PaymentMethodDesc_Header_BPR": get_payment_method_description(payment_method_code)
        ...
        "Contact_BL_Phone_QualifierDesc_L1000A_PER": get_communication_number_qualifier_description(
            contact_bl_comm1_qualifier
        ),
        "Contact_BL_Phone_L1000A_PER": contact_bl_comm1_number,
        "Contact_BL_Comm2_Qualifier_L1000A_PER": contact_bl_comm2_qualifier,
        "Contact_BL_Comm2_QualifierDesc_L1000A_PER": get_communication_number_qualifier_description(
            contact_bl_comm2_qualifier
        ),
        ...
        "Contact_CX_Phone_QualifierDesc_L1000A_PER": get_communication_number_qualifier_description(
            contact_cx_comm1_qualifier
        ),
        "Contact_CX_Phone_L1000A_PER": contact_cx_comm1_number,
        "Contact_CX_Comm2_Qualifier_L1000A_PER": contact_cx_comm2_qualifier,
        "Contact_CX_Comm2_QualifierDesc_L1000A_PER": get_communication_number_qualifier_description(
            contact_cx_comm2_qualifier
        ),
        ...
        "Contact_IC_Phone_QualifierDesc_L1000A_PER": get_communication_number_qualifier_description(
            contact_ic_comm1_qualifier
        ),
        "Contact_IC_Phone_L1000A_PER": contact_ic_comm1_number,
        "Contact_IC_Comm2_Qualifier_L1000A_PER": contact_ic_comm2_qualifier,
        "Contact_IC_Comm2_QualifierDesc_L1000A_PER": get_communication_number_qualifier_description(
            contact_ic_comm2_qualifier
        ),
        ...
        "SecondaryPayer_IDQualifierDesc_L1000A_REF": get_id_code_qualifier_description(
            secondary_payer_info.get("SecondaryPayerIDQualifier", "")
        ),
        ...
        "Payer_ContactFunctionCodeDesc_L1000A_RDM": get_contact_function_description(
            payer_rdm_data.get("contact_function_code", "")
        ),
        ...
        "Payee_ContactFunctionCodeDesc_L1000B_RDM": get_contact_function_description(
            payee_rdm_data.get("contact_function_code", "")
        ),
        ...
        "CLM_IsReversal_L2100_CLP": "Yes"
        if claim.get("claim_status", "") in ["22", "4"] and float(claim.get("total_charged", "0") or "0") < 0
        else "No",
        "CLM_ReversalIndicator_L2100_CLP": claim.get("claim_status", "")
        if claim.get("claim_status", "") in ["22", "4"]
        else "",
        # Patient condition/discharge status (CLP*10)
        ...
        "CLM_CAS1_Group_L2100_CAS": claim.get("_cas", [{}])[0].get("group_code", "")
        if claim.get("_cas") and len(claim.get("_cas", [])) > 0
        else "",
        "CLM_CAS1_Reason_L2100_CAS": claim.get("_cas", [{}])[0].get("reason_code", "")
        if claim.get("_cas") and len(claim.get("_cas", [])) > 0
        else "",
        "CLM_CAS1_Amount_L2100_CAS": claim.get("_cas", [{}])[0].get("amount", "")
        if claim.get("_cas") and len(claim.get("_cas", [])) > 0
        else "",
        "CLM_CAS1_Quantity_L2100_CAS": claim.get("_cas", [{}])[0].get("quantity", "")
        if claim.get("_cas") and len(claim.get("_cas", [])) > 0
        else "",
        "CLM_CAS1_ReasonDesc_L2100_CAS": get_carc_description(
            claim.get("_cas", [{}])[0].get("reason_code", ""), payer_key=payer_key
        )
        if claim.get("_cas") and len(claim.get("_cas", [])) > 0
        else "",
        "CLM_CAS2_Group_L2100_CAS": claim.get("_cas", [{}])[1].get("group_code", "")
        if claim.get("_cas") and len(claim.get("_cas", [])) > 1
        else "",
        "CLM_CAS2_Reason_L2100_CAS": claim.get("_cas", [{}])[1].get("reason_code", "")
        if claim.get("_cas") and len(claim.get("_cas", [])) > 1
        else "",
        "CLM_CAS2_Amount_L2100_CAS": claim.get("_cas", [{}])[1].get("amount", "")
        if claim.get("_cas") and len(claim.get("_cas", [])) > 1
        else "",
        "CLM_CAS2_Quantity_L2100_CAS": claim.get("_cas", [{}])[1].get("quantity", "")
        if claim.get("_cas") and len(claim.get("_cas", [])) > 1
        else "",
        "CLM_CAS2_ReasonDesc_L2100_CAS": get_carc_description(
            claim.get("_cas", [{}])[1].get("reason_code", ""), payer_key=payer_key
        )
        if claim.get("_cas") and len(claim.get("_cas", [])) > 1
        else "",
        "CLM_CAS3_Group_L2100_CAS": claim.get("_cas", [{}])[2].get("group_code", "")
        if claim.get("_cas") and len(claim.get("_cas", [])) > 2
        else "",
        "CLM_CAS3_Reason_L2100_CAS": claim.get("_cas", [{}])[2].get("reason_code", "")
        if claim.get("_cas") and len(claim.get("_cas", [])) > 2
        else "",
        "CLM_CAS3_Amount_L2100_CAS": claim.get("_cas", [{}])[2].get("amount", "")
        if claim.get("_cas") and len(claim.get("_cas", [])) > 2
        else "",
        "CLM_CAS3_Quantity_L2100_CAS": claim.get("_cas", [{}])[2].get("quantity", "")
        if claim.get("_cas") and len(claim.get("_cas", [])) > 2
        else "",
        "CLM_CAS3_ReasonDesc_L2100_CAS": get_carc_description(
            claim.get("_cas", [{}])[2].get("reason_code", ""), payer_key=payer_key
        )
        if claim.get("_cas") and len(claim.get("_cas", [])) > 2
        else "",
        "CLM_CAS4_Group_L2100_CAS": claim.get("_cas", [{}])[3].get("group_code", "")
        if claim.get("_cas") and len(claim.get("_cas", [])) > 3
        else "",
        "CLM_CAS4_Reason_L2100_CAS": claim.get("_cas", [{}])[3].get("reason_code", "")
        if claim.get("_cas") and len(claim.get("_cas", [])) > 3
        else "",
        "CLM_CAS4_Amount_L2100_CAS": claim.get("_cas", [{}])[3].get("amount", "")
        if claim.get("_cas") and len(claim.get("_cas", [])) > 3
        else "",
        "CLM_CAS4_Quantity_L2100_CAS": claim.get("_cas", [{}])[3].get("quantity", "")
        if claim.get("_cas") and len(claim.get("_cas", [])) > 3
        else "",
        "CLM_CAS4_ReasonDesc_L2100_CAS": get_carc_description(
            claim.get("_cas", [{}])[3].get("reason_code", ""), payer_key=payer_key
        )
        if claim.get("_cas") and len(claim.get("_cas", [])) > 3
        else "",
        "CLM_CAS5_Group_L2100_CAS": claim.get("_cas", [{}])[4].get("group_code", "")
        if claim.get("_cas") and len(claim.get("_cas", [])) > 4
        else "",
        "CLM_CAS5_Reason_L2100_CAS": claim.get("_cas", [{}])[4].get("reason_code", "")
        if claim.get("_cas") and len(claim.get("_cas", [])) > 4
        else "",
        "CLM_CAS5_Amount_L2100_CAS": claim.get("_cas", [{}])[4].get("amount", "")
        if claim.get("_cas") and len(claim.get("_cas", [])) > 4
        else "",
        "CLM_CAS5_Quantity_L2100_CAS": claim.get("_cas", [{}])[4].get("quantity", "")
        if claim.get("_cas") and len(claim.get("_cas", [])) > 4
        else "",
        "CLM_CAS5_ReasonDesc_L2100_CAS": get_carc_description(
            claim.get("_cas", [{}])[4].get("reason_code", ""), payer_key=payer_key
        )
        if claim.get("_cas") and len(claim.get("_cas", [])) > 4
        else "",
        ...
        "CLM_CorrectedInsuredIDQualifierDesc_L2100_NM1": get_id_code_qualifier_description(
            corrected_insured_id_qualifier
        ),
        ...
        "CLM_DTM_Times_L2100_DTM": ", ".join([d.get("time", "") for d in claim.get("_dtm", []) if d.get("time", "")])
        if claim
        else "",
        "CLM_DTM_TimeCodes_L2100_DTM": ", ".join(
            [d.get("time_code", "") for d in claim.get("_dtm", []) if d.get("time_code", "")]
        )
        if claim
        else "",
        "CLM_DTM_PeriodFormats_L2100_DTM": ", ".join(
            [
                d.get("date_time_period_format", "")
                for d in claim.get("_dtm", [])
                if d.get("date_time_period_format", "")
            ]
        )
        if claim
        else "",
        "CLM_DTM_Periods_L2100_DTM": ", ".join(
            [d.get("date_time_period", "") for d in claim.get("_dtm", []) if d.get("date_time_period", "")]
        )
        if claim
        else "",
        ...
        "CLM_HealthcareRemarkDescriptions_L2100_LQ": "; ".join(
            [
                get_remark_code_description(c, payer_key=payer_key)
                for c in extract_lq_codes_by_qualifier(claim.get("_lq", []), "HE")
            ]
        ),
        # QTY*CA - Covered Actual Days/Units
        "CLM_CoveredActual_L2100_QTY": extract_qty_value(claim.get("_qty", []), "CA"),
        "CLM_ContactFunction_L2100_PER": claim.get("_per", [{}])[0].get("contact_function_code", "")
        if claim.get("_per")
        else "",
        "CLM_ContactFunctionDesc_L2100_PER": get_contact_function_description(
            claim.get("_per", [{}])[0].get("contact_function_code", "")
        )
        if claim.get("_per")
        else "",
        "CLM_ContactName_L2100_PER": claim.get("_per", [{}])[0].get("name", "") if claim.get("_per") else "",
        "CLM_ContactPhone_Qualifier_L2100_PER": claim.get("_per", [{}])[0]
        .get("communication", [{}])[0]
        .get("qualifier", "")
        if claim.get("_per") and claim.get("_per", [{}])[0].get("communication")
        else "",
        "CLM_ContactPhone_QualifierDesc_L2100_PER": get_communication_number_qualifier_description(
            claim.get("_per", [{}])[0].get("communication", [{}])[0].get("qualifier", "")
        )
        if claim.get("_per") and claim.get("_per", [{}])[0].get("communication")
        else "",
        "CLM_ContactPhone_L2100_PER": claim.get("_per", [{}])[0].get("communication", [{}])[0].get("number", "")
        if claim.get("_per") and claim.get("_per", [{}])[0].get("communication")
        else "",
        "ServiceProvider_EntityCode_L2100_NM1": claim.get("_service_provider", {}).get("entity_code", ""),
        "ServiceProvider_EntityCodeDesc_L2100_NM1": get_entity_identifier_description(
            claim.get("_service_provider", {}).get("entity_code", ""), payer_key=payer_key
        ),
        ...
        "ServiceProvider_IDQualifierDesc_L2100_NM1": get_id_code_qualifier_description(
            claim.get("_service_provider", {}).get("id_qualifier", "")
        ),
        "ServiceProvider_IDCode_L2100_NM1": claim.get("_service_provider", {}).get("id_code", ""),
        "ServiceProvider_EntityRelationship_L2100_NM1": claim.get("_service_provider", {}).get(
            "entity_relationship", ""
        ),
        "ServiceProvider_EntityRelationshipDesc_L2100_NM1": get_entity_relationship_description(
            claim.get("_service_provider", {}).get("entity_relationship", "")
        ),
        "ServiceProvider_EntityIDSecondary_L2100_NM1": claim.get("_service_provider", {}).get(
            "entity_id_secondary", ""
        ),
        "SVC_ProcedureCode_L2110_SVC": service.get("procedure_code", "") if service else "",
        "SVC_CodeDescription_L2110_SVC": get_ambulance_code_description(service.get("procedure_code", ""))
        if service and is_transportation_code(service.get("procedure_code", ""))
        else "",
        "SVC_ServiceLevel_L2110_SVC": get_ambulance_service_level_name(service.get("procedure_code", ""))
        if service and is_transportation_code(service.get("procedure_code", ""))
        else "",
        ...
        "SVC_ModifierDescriptions_L2110_SVC": "; ".join(
            [get_ambulance_modifier_description(m) for m in service.get("modifiers", [])]
        )
        if service
        else "",
        ...
        "SVC_CAS1_Group_L2110_CAS": service["_cas"][0]["group_code"]
        if service and "_cas" in service and len(service["_cas"]) > 0
        else "",
        "SVC_CAS1_GroupDesc_L2110_CAS": get_cas_group_code_description(service["_cas"][0]["group_code"])
        if service and "_cas" in service and len(service["_cas"]) > 0
        else "",
        "SVC_CAS1_Reason_L2110_CAS": service["_cas"][0]["reason_code"]
        if service and "_cas" in service and len(service["_cas"]) > 0
        else "",
        "SVC_CAS1_Amount_L2110_CAS": service["_cas"][0]["amount"]
        if service and "_cas" in service and len(service["_cas"]) > 0
        else "",
        "SVC_CAS1_ReasonDesc_L2110_CAS": get_carc_description(service["_cas"][0]["reason_code"], payer_key=payer_key)
        if service and "_cas" in service and len(service["_cas"]) > 0
        else "",
        "SVC_CAS2_Group_L2110_CAS": service["_cas"][1]["group_code"]
        if service and "_cas" in service and len(service["_cas"]) > 1
        else "",
        "SVC_CAS2_GroupDesc_L2110_CAS": get_cas_group_code_description(service["_cas"][1]["group_code"])
        if service and "_cas" in service and len(service["_cas"]) > 1
        else "",
        "SVC_CAS2_Reason_L2110_CAS": service["_cas"][1]["reason_code"]
        if service and "_cas" in service and len(service["_cas"]) > 1
        else "",
        "SVC_CAS2_Amount_L2110_CAS": service["_cas"][1]["amount"]
        if service and "_cas" in service and len(service["_cas"]) > 1
        else "",
        "SVC_CAS2_ReasonDesc_L2110_CAS": get_carc_description(service["_cas"][1]["reason_code"], payer_key=payer_key)
        if service and "_cas" in service and len(service["_cas"]) > 1
        else "",
        "SVC_CAS3_Group_L2110_CAS": service["_cas"][2]["group_code"]
        if service and "_cas" in service and len(service["_cas"]) > 2
        else "",
        "SVC_CAS3_GroupDesc_L2110_CAS": get_cas_group_code_description(service["_cas"][2]["group_code"])
        if service and "_cas" in service and len(service["_cas"]) > 2
        else "",
        "SVC_CAS3_Reason_L2110_CAS": service["_cas"][2]["reason_code"]
        if service and "_cas" in service and len(service["_cas"]) > 2
        else "",
        "SVC_CAS3_Amount_L2110_CAS": service["_cas"][2]["amount"]
        if service and "_cas" in service and len(service["_cas"]) > 2
        else "",
        "SVC_CAS3_ReasonDesc_L2110_CAS": get_carc_description(service["_cas"][2]["reason_code"], payer_key=payer_key)
        if service and "_cas" in service and len(service["_cas"]) > 2
        else "",
        "SVC_CAS4_Group_L2110_CAS": service["_cas"][3]["group_code"]
        if service and "_cas" in service and len(service["_cas"]) > 3
        else "",
        "SVC_CAS4_GroupDesc_L2110_CAS": get_cas_group_code_description(service["_cas"][3]["group_code"])
        if service and "_cas" in service and len(service["_cas"]) > 3
        else "",
        "SVC_CAS4_Reason_L2110_CAS": service["_cas"][3]["reason_code"]
        if service and "_cas" in service and len(service["_cas"]) > 3
        else "",
        "SVC_CAS4_Amount_L2110_CAS": service["_cas"][3]["amount"]
        if service and "_cas" in service and len(service["_cas"]) > 3
        else "",
        "SVC_CAS4_ReasonDesc_L2110_CAS": get_carc_description(service["_cas"][3]["reason_code"], payer_key=payer_key)
        if service and "_cas" in service and len(service["_cas"]) > 3
        else "",
        "SVC_CAS5_Group_L2110_CAS": service["_cas"][4]["group_code"]
        if service and "_cas" in service and len(service["_cas"]) > 4
        else "",
        "SVC_CAS5_GroupDesc_L2110_CAS": get_cas_group_code_description(service["_cas"][4]["group_code"])
        if service and "_cas" in service and len(service["_cas"]) > 4
        else "",
        "SVC_CAS5_Reason_L2110_CAS": service["_cas"][4]["reason_code"]
        if service and "_cas" in service and len(service["_cas"]) > 4
        else "",
        "SVC_CAS5_Amount_L2110_CAS": service["_cas"][4]["amount"]
        if service and "_cas" in service and len(service["_cas"]) > 4
        else "",
        ...
        "SVC_RepricedLineItemRefNumber_L2110_REF": service_ref_values.get("repriced_line_item_ref_number", "")
        if service
        else "",
        "SVC_AmbulatoryPaymentClassification_L2110_REF": service_ref_values.get("ambulatory_payment_classification", "")
        if service
        else "",
        ...
        "SVC_DTM_Times_L2110_DTM": ", ".join([d.get("time", "") for d in service.get("_dtm", []) if d.get("time", "")])
        if service
        else "",
        "SVC_DTM_TimeCodes_L2110_DTM": ", ".join(
            [d.get("time_code", "") for d in service.get("_dtm", []) if d.get("time_code", "")]
        )
        if service
        else "",
        "SVC_DTM_PeriodFormats_L2110_DTM": ", ".join(
            [
                d.get("date_time_period_format", "")
                for d in service.get("_dtm", [])
                if d.get("date_time_period_format", "")
            ]
        )
        if service
        else "",
        "SVC_DTM_Periods_L2110_DTM": ", ".join(
            [d.get("date_time_period", "") for d in service.get("_dtm", []) if d.get("date_time_period", "")]
        )
        if service
        else "",
        ...
        # LQ*HE - Healthcare Remark Codes (RARC codes)
        "SVC_HealthcareRemarkCodes_L2110_LQ": ", ".join(extract_lq_codes_by_qualifier(service.get("_lq", []), "HE"))
        if service
        else "",
        "SVC_HealthcareRemarkDescriptions_L2110_LQ": "; ".join(
            [
                get_remark_code_description(c, payer_key=payer_key)
                for c in extract_lq_codes_by_qualifier(service.get("_lq", []), "HE")
            ]
        )
        if service
        else "",
        ...
        "MOA_ReimbursementRate_L2100_MOA": claim.get("_moa", [{}])[0].get("reimbursement_rate", "")
        if claim.get("_moa")
        else "",
        "MOA_ClaimHCPCSPayableAmount_L2100_MOA": claim.get("_moa", [{}])[0].get("claim_hcpcs_payable_amount", "")
        if claim.get("_moa")
        else "",
        "MOA_ClaimPaymentRemarkCode1_L2100_MOA": claim.get("_moa", [{}])[0].get("claim_payment_remark_code_1", "")
        if claim.get("_moa")
        else "",
        "MOA_ClaimPaymentRemarkCode2_L2100_MOA": claim.get("_moa", [{}])[0].get("claim_payment_remark_code_2", "")
        if claim.get("_moa")
        else "",
        "MOA_ClaimPaymentRemarkCode3_L2100_MOA": claim.get("_moa", [{}])[0].get("claim_payment_remark_code_3", "")
        if claim.get("_moa")
        else "",
        "MOA_ClaimPaymentRemarkCode4_L2100_MOA": claim.get("_moa", [{}])[0].get("claim_payment_remark_code_4", "")
        if claim.get("_moa")
        else "",
        "MOA_ClaimPaymentRemarkCode5_L2100_MOA": claim.get("_moa", [{}])[0].get("claim_payment_remark_code_5", "")
        if claim.get("_moa")
        else "",
        "MOA_ClaimPaymentRemarkDesc1_L2100_MOA": get_remark_code_description(
            claim.get("_moa", [{}])[0].get("claim_payment_remark_code_1", ""), payer_key=payer_key
        )
        if claim.get("_moa") and claim.get("_moa", [{}])[0].get("claim_payment_remark_code_1")
        else "",
        "MOA_ClaimPaymentRemarkDesc2_L2100_MOA": get_remark_code_description(
            claim.get("_moa", [{}])[0].get("claim_payment_remark_code_2", ""), payer_key=payer_key
        )
        if claim.get("_moa") and claim.get("_moa", [{}])[0].get("claim_payment_remark_code_2")
        else "",
        "MOA_ClaimPaymentRemarkDesc3_L2100_MOA": get_remark_code_description(
            claim.get("_moa", [{}])[0].get("claim_payment_remark_code_3", ""), payer_key=payer_key
        )
        if claim.get("_moa") and claim.get("_moa", [{}])[0].get("claim_payment_remark_code_3")
        else "",
        "MOA_ClaimPaymentRemarkDesc4_L2100_MOA": get_remark_code_description(
            claim.get("_moa", [{}])[0].get("claim_payment_remark_code_4", ""), payer_key=payer_key
        )
        if claim.get("_moa") and claim.get("_moa", [{}])[0].get("claim_payment_remark_code_4")
        else "",
        "MOA_ClaimPaymentRemarkDesc5_L2100_MOA": get_remark_code_description(
            claim.get("_moa", [{}])[0].get("claim_payment_remark_code_5", ""), payer_key=payer_key
        )
        if claim.get("_moa") and claim.get("_moa", [{}])[0].get("claim_payment_remark_code_5")
        else "",
        "MOA_ESRDPaymentAmount_L2100_MOA": claim.get("_moa", [{}])[0].get("esrd_payment_amount", "")
        if claim.get("_moa")
        else "",
        "MOA_NonpayableProfessionalComponent_L2100_MOA": claim.get("_moa", [{}])[0].get(
            "nonpayable_professional_component", ""
        )
        if claim.get("_moa")
        else "",
        # MIA fields (Medicare Inpatient Adjudication)
        "MIA_CoveredDaysOrVisitsCount_L2100_MIA": claim.get("_mia", [{}])[0].get("covered_days_or_visits_count", "")
        if claim.get("_mia")
        else "",
        "MIA_PPSOperatingOutlierAmount_L2100_MIA": claim.get("_mia", [{}])[0].get("pps_operating_outlier_amount", "")
        if claim.get("_mia")
        else "",
        "MIA_LifetimePsychiatricDaysCount_L2100_MIA": claim.get("_mia", [{}])[0].get(
            "lifetime_psychiatric_days_count", ""
        )
        if claim.get("_mia")
        else "",
        "MIA_ClaimDRGAmount_L2100_MIA": claim.get("_mia", [{}])[0].get("claim_drg_amount", "")
        if claim.get("_mia")
        else "",
        "MIA_ClaimPaymentRemarkCode_L2100_MIA": claim.get("_mia", [{}])[0].get("claim_payment_remark_code", "")
        if claim.get("_mia")
        else "",
        "MIA_ClaimPaymentRemarkDesc_L2100_MIA": get_remark_code_description(
            claim.get("_mia", [{}])[0].get("claim_payment_remark_code", ""), payer_key=payer_key
        )
        if claim.get("_mia") and claim.get("_mia", [{}])[0].get("claim_payment_remark_code")
        else "",
        "MIA_ClaimDisproportionateShareAmount_L2100_MIA": claim.get("_mia", [{}])[0].get(
            "claim_disproportionate_share_amount", ""
        )
        if claim.get("_mia")
        else "",
        "MIA_ClaimMSPPassThroughAmount_L2100_MIA": claim.get("_mia", [{}])[0].get("claim_msp_pass_through_amount", "")
        if claim.get("_mia")
        else "",
        "MIA_ClaimPPSCapitalAmount_L2100_MIA": claim.get("_mia", [{}])[0].get("claim_pps_capital_amount", "")
        if claim.get("_mia")
        else "",
        "MIA_PPSCapitalFSPDRGAmount_L2100_MIA": claim.get("_mia", [{}])[0].get("pps_capital_fsp_drg_amount", "")
        if claim.get("_mia")
        else "",
        "MIA_PPSCapitalHSPDRGAmount_L2100_MIA": claim.get("_mia", [{}])[0].get("pps_capital_hsp_drg_amount", "")
        if claim.get("_mia")
        else "",
        "MIA_PPSCapitalDSHDRGAmount_L2100_MIA": claim.get("_mia", [{}])[0].get("pps_capital_dsh_drg_amount", "")
        if claim.get("_mia")
        else "",
        "MIA_OldCapitalAmount_L2100_MIA": claim.get("_mia", [{}])[0].get("old_capital_amount", "")
        if claim.get("_mia")
        else "",
        "MIA_PPSCapitalIMEAmount_L2100_MIA": claim.get("_mia", [{}])[0].get("pps_capital_ime_amount", "")
        if claim.get("_mia")
        else "",
        "MIA_PPSOperatingHospitalSpecificDRGAmount_L2100_MIA": claim.get("_mia", [{}])[0].get(
            "pps_operating_hospital_specific_drg_amount", ""
        )
        if claim.get("_mia")
        else "",
        "MIA_CostReportDayCount_L2100_MIA": claim.get("_mia", [{}])[0].get("cost_report_day_count", "")
        if claim.get("_mia")
        else "",
        "MIA_PPSOperatingFederalSpecificDRGAmount_L2100_MIA": claim.get("_mia", [{}])[0].get(
            "pps_operating_federal_specific_drg_amount", ""
        )
        if claim.get("_mia")
        else "",
        "MIA_ClaimPPSCapitalOutlierAmount_L2100_MIA": claim.get("_mia", [{}])[0].get(
            "claim_pps_capital_outlier_amount", ""
        )
        if claim.get("_mia")
        else "",
        "MIA_ClaimIndirectTeachingAmount_L2100_MIA": claim.get("_mia", [{}])[0].get(
            "claim_indirect_teaching_amount", ""
        )
        if claim.get("_mia")
        else "",
        "MIA_NonpayableProfessionalComponentAmount_L2100_MIA": claim.get("_mia", [{}])[0].get(
            "nonpayable_professional_component_amount", ""
        )
        if claim.get("_mia")
        else "",
        "MIA_ClaimPaymentRemarkCode2_L2100_MIA": claim.get("_mia", [{}])[0].get("claim_payment_remark_code_2", "")
        if claim.get("_mia")
        else "",
        "MIA_ClaimPaymentRemarkCode3_L2100_MIA": claim.get("_mia", [{}])[0].get("claim_payment_remark_code_3", "")
        if claim.get("_mia")
        else "",
        "MIA_ClaimPaymentRemarkCode4_L2100_MIA": claim.get("_mia", [{}])[0].get("claim_payment_remark_code_4", "")
        if claim.get("_mia")
        else "",
        "MIA_ClaimPaymentRemarkCode5_L2100_MIA": claim.get("_mia", [{}])[0].get("claim_payment_remark_code_5", "")
        if claim.get("_mia")
        else "",
        "MIA_ClaimPaymentRemarkDesc2_L2100_MIA": get_remark_code_description(
            claim.get("_mia", [{}])[0].get("claim_payment_remark_code_2", ""), payer_key=payer_key
        )
        if claim.get("_mia") and claim.get("_mia", [{}])[0].get("claim_payment_remark_code_2")
        else "",
        "MIA_ClaimPaymentRemarkDesc3_L2100_MIA": get_remark_code_description(
            claim.get("_mia", [{}])[0].get("claim_payment_remark_code_3", ""), payer_key=payer_key
        )
        if claim.get("_mia") and claim.get("_mia", [{}])[0].get("claim_payment_remark_code_3")
        else "",
        "MIA_ClaimPaymentRemarkDesc4_L2100_MIA": get_remark_code_description(
            claim.get("_mia", [{}])[0].get("claim_payment_remark_code_4", ""), payer_key=payer_key
        )
        if claim.get("_mia") and claim.get("_mia", [{}])[0].get("claim_payment_remark_code_4")
        else "",
        "MIA_ClaimPaymentRemarkDesc5_L2100_MIA": get_remark_code_description(
            claim.get("_mia", [{}])[0].get("claim_payment_remark_code_5", ""), payer_key=payer_key
        )
        if claim.get("_mia") and claim.get("_mia", [{}])[0].get("claim_payment_remark_code_5")
        else "",
        "MIA_PPSCapitalExceptionAmount_L2100_MIA": claim.get("_mia", [{}])[0].get("pps_capital_exception_amount", "")
        if claim.get("_mia")
        else "",
        # Ambulance Location Data
        "AMB_PickupName_L2100_NM1": claim.get("_ambulance_pickup", {}).get("last_name", "")
        if claim.get("_ambulance_pickup")
        else "",
        "AMB_PickupAddress_L2100_N3": claim.get("_ambulance_pickup_address", {}).get("address_line_1", "")
        if claim.get("_ambulance_pickup_address")
        else "",
        ...
        "AMB_DropoffName_L2100_NM1": claim.get("_ambulance_dropoff", {}).get("last_name", "")
        if claim.get("_ambulance_dropoff")
        else "",
        "AMB_DropoffAddress_L2100_N3": claim.get("_ambulance_dropoff_address", {}).get("address_line_1", "")
        if claim.get("_ambulance_dropoff_address")
        else "",
        ...
        "Allowed_Amount": f"{(float(service.get('line_charged', 0) or 0) - service_adjustments['Contractual'] - service_adjustments['COB'] - service_adjustments['Sequestration'] - service_adjustments['HCRA'] - service_adjustments['OtherAdjustments'] - service_adjustments['Denied'] - service_adjustments['QMB']):.2f}"
        if service
        else "",
        "Allowed_Verification": f"{(float(service.get('line_paid', 0) or 0) + service_adjustments['Deductible'] + service_adjustments['Coinsurance'] + service_adjustments['Copay'] + service_adjustments['PR_NonCovered'] + service_adjustments['OtherPatientResp']):.2f}"
        if service
        else "",
        ...
        "RDM_TransmissionCode_Header_RDM": current_transaction.get("remittance_delivery", {}).get(
            "report_transmission_code", ""
        )
        if current_transaction
        else "",
        "RDM_Name_Header_RDM": current_transaction.get("remittance_delivery", {}).get("name", "")
        if current_transaction
        else "",
        "RDM_CommunicationNumber_Header_RDM": current_transaction.get("remittance_delivery", {}).get(
            "communication_number", ""
        )
        if current_transaction
        else "",
        "RDM_ReferenceID_Header_RDM": current_transaction.get("remittance_delivery", {}).get("reference_identifier", "")
        if current_transaction
        else "",
        "RDM_ReferenceID2_Header_RDM": current_transaction.get("remittance_delivery", {}).get(
            "reference_identifier_2", ""
        )
        if current_transaction
        else "",
        "RDM_CommunicationNumber2_Header_RDM": current_transaction.get("remittance_delivery", {}).get(
            "communication_number_2", ""
        )
        if current_transaction
        else "",
        "RDM_ContactFunction_Header_RDM": current_transaction.get("remittance_delivery", {}).get(
        ...
        if rate_tuple:
            ...
            if is_mileage_code:
                ...
                try:
                    if oon_rate:
                        oon_unit_price = str(
                            (Decimal(str(oon_rate)) / base_units).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
                        )
                except (ValueError, TypeError, ArithmeticError):
                    pass
                try:
                    if in_rate:
                        in_unit_price = str(
                            (Decimal(str(in_rate)) / base_units).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
                        )
                except (ValueError, TypeError, ArithmeticError):
                    pass
                ...
                if oon_unit_price and service_units_decimal > 0:
                    try:
                        oon_miles = str(
                            (Decimal(str(oon_unit_price)) * service_units_decimal).quantize(
                ...
                if in_unit_price and service_units_decimal > 0:
                    try:
                        in_miles = str(
                            (Decimal(str(in_unit_price)) * service_units_decimal).quantize(
                                Decimal("0.01"), rounding=ROUND_HALF_UP
                            )
                        )
                    except (ValueError, TypeError, ArithmeticError, InvalidOperation):
...
def process_folder(folder_path, enable_redaction=False, status_callback=None):
            ...
            if parsed_data:
                ...
                try:
                    ...
                    for line in traceback.format_exception(
                        type(convert_error), convert_error, convert_error.__traceback__
                    ):
    ...
    if normalized_rows and config.get("enable_compact_csv", True):
        ...
        populated_columns = [
            ...
            if col in populated_columns_set
            and not any(pattern.lower() in col.lower() for pattern in compact_excluded_patterns)
        ]
        ...
        if removed_count > 0:
            ...
            logger.info("  Total service lines: %d", len(normalized_rows))
            logger.info(
                "  Columns: %d of %d (%d empty columns removed)",
    ...
    try:
        ...
        if validation_data and all_rows:
            ...
            generate_validation_report(
                validation_result, output_format="html", output_file=str(validation_html), redact=enable_redaction
            )
            ...
            if errors > 0:
                ...
                state_provider_payer_type_errors = defaultdict(
                ...
                for error in all_errors:
                    ...
                    state_provider_payer_type_errors[provider_state][provider_name][payer_name][error_type].append(
                        error
                    )
                ...
                for state, providers in state_provider_payer_type_errors.items():
                    total = sum(
                        sum(sum(len(errs) for errs in types.values()) for types in payers.values())
                        for payers in providers.values()
                    )
                        ...
                        for payer in sorted(payer_totals.keys(), key=lambda p: payer_totals[p], reverse=True):
                            ...
                            for error_type in sorted(types.keys(), key=lambda t: len(types[t]), reverse=True):
                                ...
                                if error_type == "COMPOSITE_PARSE" and len(type_errors) > 5:
                                    ...
                                    logger.info(
                                        "        Summary: %d claims with composite errors", len(claims_affected)
                                    )
                                    for i, (claim_id, service_count) in enumerate(
                                        list(claims_affected.items())[:10], 1
                                    ):
        ...
        if unmapped_elements:
            ...
            for key, info in unmapped_elements.items():
                ...
                if info["files_with_data"] >= 5 or info["pct"] >= 1.0:
                    logger.info(
                        "  %s: %d files (%s%%) - %d total occurrences",
                        key,
                        info["files_with_data"],
                        info["pct"],
